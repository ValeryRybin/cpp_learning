// Для того, чтобы понимать, что является временным объектом, а что нет, нам нужна какая-то новая синтаксическая конструкция
...
// Важно: на самом деле все конечно лежит в памяти, на это нужно смотреть с точки зрения кода, а не с точки зрения работы с памятью
...
int main() {
    ...
    // если говорить проще, то нам нужно отличить результат выражения 
    // от переменной, которая уже лежит в памяти 
    // на помощь приходят rvalue ссылки
}
